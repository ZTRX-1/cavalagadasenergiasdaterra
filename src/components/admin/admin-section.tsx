...
export function AdminSection({
  ...
  return (
    <section className="admin-card p-4 md:p-6 lg:p-7">
      <header className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <h2 className="font-display text-lg text-[color:var(--admin-cinza-1)] md:text-xl">{titulo}</h2>
          {descricao ? (
            <p className="mt-1 text-[11px] leading-relaxed text-[color:var(--admin-cinza-3)] md:text-xs">{descricao}</p>
          ) : null}
        </div>
