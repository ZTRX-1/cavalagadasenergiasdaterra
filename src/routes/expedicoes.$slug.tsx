...
function DetalhesExpedicao() {
  ...
  const fallbackHeroImg = getExpedicaoImage(expedicao.slug);
    ...
    <>
      ...
      <section className="relative text-areia min-h-[78svh] md:min-h-[62svh] lg:min-h-[64svh]">
        <img
          src={heroImg}
          alt={expedicao.nome}
          className={`absolute inset-0 h-full w-full object-cover ${isJeri ? "object-[center_28%]" : ""}`}
          fetchPriority="high"
          onError={(event) => {
        ...
        <div className="container-tight relative flex flex-col justify-end pb-14 pt-32 min-h-[78svh] md:min-h-[62svh] lg:min-h-[64svh]">
          ...
          <div className="flex items-center gap-3 font-eyebrow text-[0.7rem] uppercase tracking-[0.22em] text-areia/80">
            ...
            <span className="h-1 w-1 rounded-full bg-cobre" />
            <span>{expedicao.mensagem_comercial_publica ? expedicao.mensagem_comercial_publica : (expedicao.ativo ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(expedicao.preco) : t("expedicoes.consulteValores"))}</span>
          </div>
