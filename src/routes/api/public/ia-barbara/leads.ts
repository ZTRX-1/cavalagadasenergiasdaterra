...
async function handleGET(request: Request): Promise<Response> {
  ...
  if (action === "expedicoes") {
    ...
    const datasByExp = new Map<string, NonNullable<typeof datas>>();
    (datas ?? []).forEach((d) => {
...
async function handlePOST(request: Request): Promise<Response> {
  ...
  if (body.action === "upsert_lead") {
    ...
    if (body.telefone) {
      const { data } = await tbl("leads").select("id").eq("telefone", body.telefone).maybeSingle();
      existing = (data as { id: string } | null) ?? null;
    }
    if (!existing && body.email) {
      const { data } = await tbl("leads").select("id").eq("email", body.email).maybeSingle();
      existing = (data as { id: string } | null) ?? null;
    }
    ...
    if (existing) {
      ...
      const { data, error } = await tbl("leads").update(patch as never).eq("id", existing.id).select().single();
      if (error) return json({ error: error.message }, { status: 500 });
    ...
    const { data, error } = await tbl("leads").insert(payload as never).select().single();
    if (error) return json({ error: error.message }, { status: 500 });
  ...
  if (body.action === "update_campos") {
    ...
    const { data, error } = await tbl("leads").update(cleaned as never).eq("id", lead_id).select().single();
    if (error) return json({ error: error.message }, { status: 500 });
  ...
  if (body.action === "registrar_conversa") {
    ...
    const { data, error } = await tbl("lead_conversas").insert(payload as never).select().single();
    if (error) return json({ error: error.message }, { status: 500 });
    await tbl("leads").update({ ultima_interacao_at: new Date().toISOString() } as never).eq("id", body.lead_id);
  ...
  if (body.action === "transferir_humano") {
    const patch: Record<string, unknown> = {
      ...
      proxima_acao: body.motivo ?? "Atendimento humano solicitado",
    };
    const { data, error } = await tbl("leads").update(patch as never).eq("id", body.lead_id).select().single();
    if (error) return json({ error: error.message }, { status: 500 });
    return json({ lead: data });
  }
